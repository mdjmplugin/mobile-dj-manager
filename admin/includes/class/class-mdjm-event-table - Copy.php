	...
	class MDJM_Events_Table extends WP_List_Table	{
		...
		function column_event_date( $item ) {
			...
			if( $item['contract_status'] != 'Cancelled' && $item['contract_status'] != 'Completed' && $item['contract_status'] != 'Enquiry' && $item['contract_status'] != 'Unattended' && $item['contract_status'] != 'Failed Enquiry' )	{
				...
				$actions['cancel'] = sprintf( '<span class="trash"><a href="?page=%s&action=%s&event_id=%s">Cancel</a></span>', $_REQUEST['page'], 'cancel_event', $item['event_id'] );
			}
			if( $item['contract_status'] == 'Enquiry' )	{
				...
				$actions['failed'] = sprintf( '<span class="trash"><a href="?page=%s&action=%s&event_id=%s">Fail</a></span>', $_REQUEST['page'], 'fail_enquiry', $item['event_id'] );
			}
			if( $item['contract_status'] == 'Unattended' )	{
