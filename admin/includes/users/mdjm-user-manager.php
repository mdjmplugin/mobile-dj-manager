...
/**
 * Class Name: MDJM_User_Manager
 * User management interface for employees
 *
 *
 *
 */
if( !class_exists( 'MDJM_User_Manager' ) ) : 
	class MDJM_User_Manager	{
		...
		private static $mdjm_user_table;
		/**
		 * Init
		 *
		 *
		...
		public static function init()	{
			...
			if( self::$active_tab == 'permissions' )	{
				...
				self::$mdjm_user_table = new MDJM_User_Table();
				self::$mdjm_user_table->process_bulk_actions();
				...
				self::$mdjm_user_table->prepare_items();
		...
		public static function employee_page()	{
			...
			self::$mdjm_user_table->views();
			?>
			<table style="width: 100%;">
			<tr valign="top">
			<td style="width: auto;"><?php self::$mdjm_user_table->display(); ?></td>
		...
		public static function add_employee_form()	{
			...
			<select name="employee_role" id="employee_role" required>
                ...
                <?php 
                add_filter( 'mdjm_user_roles', array( 'MDJM_User_Manager', 'no_admin_role' ) );
                echo MDJM()->roles->roles_dropdown();
                remove_filter( 'mdjm_user_roles', array( 'MDJM_User_Manager', 'no_admin_role' ) );
                ?>
            </select>
            </td>
            </tr>
	...
	class MDJM_User_Table extends WP_List_Table {
		...
		public function get_columns(){
			$columns = array(
				...
				'role'	  => __( 'Role', 'mobile-dj-manager' ),
		...
		public function prepare_items() {
			// The data is prepared in the MDJM_User_Manager class
			$employees = MDJM_User_Manager::$employees;
		...
		public function column_default( $item, $column_name ) {
			...
			switch( $column_name ) { 
				...
				case 'role':
					echo ( !empty( $item->roles[0] ) ? translate_user_role( $wp_roles->roles[$item->roles[0]]['name'] ) : __( 'No role assigned' ) );
				break;
				...
				case 'login':
					echo ( '' != get_user_meta( $item->ID, 'last_login', true ) ? date( 'H:i d M Y', strtotime( get_user_meta( $item->ID, 'last_login', true ) ) ) : __( 'Never', 'mobile-dj-manager' ) );
				break;
				default:
					return;
			  }
		...
		/**
		 * Create the HTML output for the checkboxes
		 *
		...
		public function column_cb( $item ) {
			return sprintf(
				'<input type="checkbox" name="employees[]" id="employees-' . $item->ID . '" value="%s" />', $item->ID );    
		} // column_cb
		...
		public function get_bulk_actions() {
			$actions = array(
				'delete'    => __( 'Delete', 'mobile-dj-manager' ) );
