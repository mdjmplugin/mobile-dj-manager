...
<div id="mdjm_playlist_wrap">
    ...
    <div id="mdjm_playlist_form_wrap" class="mdjm_clearfix">
        ...
        <form id="mdjm_playlist_form" class="mdjm_form" method="post">
            ...
            <fieldset id="mdjm_playlist_form_fields">
                ...
                <?php endif; ?>
                <p class="view_current_playlist<?php echo esc_attr( $view_playlist_class ); ?>"> <a class="mdjm-scroller" href="#client-playlist-entries"><?php echo esc_html( $view_playlist ); ?></a> </p>
                <div class="mdjm-alert mdjm-alert-success mdjm-hidden"></div>
                <div id="mdjm-playlist-input-fields">
                    <p class="mdjm_artist_field">
                        <label for="mdjm_artist"><?php echo esc_attr( $artist_label ); ?></label>
                    ...
                    <p class="mdjm_song_field">
                        <label for="mdjm_song"><?php echo esc_attr( $song_label ); ?> <span class="mdjm-required-indicator">*</span></label>
                    ...
                    <p class="mdjm_category_field">
                        <label for="mdjm_category"><?php echo esc_attr( $category_label ); ?></label>
                    ...
                    <p class="mdjm_notes_field">
                        <label for="mdjm_notes"><?php echo esc_attr( $notes_label ); ?></label>
            ...
            <?php // Check if the class and function exist before including the file and calling the function
            if ( class_exists( 'MDJM_SP_Converter' ) ) {
                // Get the plugin directory path for mdjm-spotify-playlist-converter and include the file directly
                require_once WP_PLUGIN_DIR . '/mdjm-spotify-playlist-converter/includes/spc-functions.php';

                // Call the function to display the Spotify playlist converter form
                mdjm_spotify_playlist_converter();
            }
            ?>
        </form>
    ...
    <div id="playlist-entries"<?php echo $entries_class; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>>
        ...
        <p>
            <?php
            printf(
                __( 'Your playlist currently consists of <span class="song-count">%1$d %2$s</span> and is approximately <span class="playlist-length">%3$s</span> long. Your %4$s is scheduled for %5$s.', 'mobile-dj-manager' ),
        ...
        <div class="playlist-entry-row-headings">
            <div class="playlist-entry-column"><span class="playlist-entry-heading"><?php echo esc_html( $artist_label ); ?></span></div>
            <div class="playlist-entry-column"><span class="playlist-entry-heading"><?php echo esc_html( $song_label ); ?></span></div>
            <div class="playlist-entry-column"><span class="playlist-entry-heading"></span></div>
        </div>
        <?php foreach ( $playlist_entries as $category => $category_entries ) : ?>
            <?php foreach ( $category_entries as $entry ) : ?>
                ...
                <div class="playlist-entry-row mdjm-playlist-entry-<?php echo esc_attr( $entry->ID ); ?>">
                    <div class="playlist-entry-column"><span class="playlist-entry"><?php echo esc_attr( $entry_data['artist'] ); ?></span></div>
                    <div class="playlist-entry-column"><span class="playlist-entry"><?php echo esc_attr( $entry_data['song'] ); ?></span></div>
                    <div class="playlist-entry-column"><span class="playlist-entry"><?php echo esc_attr( $category ); ?></span></div>
