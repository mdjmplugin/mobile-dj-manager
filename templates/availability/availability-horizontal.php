<?php
/**
 * This template is used to display the availability form with shortcode [mdjm-availability display="hortizontal"].
 *
 * @version			1.0
 * @author			Mike Howard
 * @since			1.3
...
<div id="mdjm-availability-checker">
    ...
    <input type="text" name="{field}" id="{field}" class="{field_class}" size="20" placeholder="<?php echo mdjm_format_datepicker_date(); ?>" />
    <input type="submit" name="mdjm-submit-availability" id="mdjm-submit-availability" class="{submit_class}" value="{submit_text}" /></p>
	<span id="pleasewait" class="{please_wait_class}">{please_wait_text} <img src="/wp-admin/images/loading.gif" alt="{please_wait_text}" /></span>
</div>
