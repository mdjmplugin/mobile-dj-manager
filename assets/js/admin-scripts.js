 ...
 jQuery( document ).ready(
 	function ($) {
		...
		$( '.mdjm-select-chosen .chosen-search input' ).each(
			function() {
		...
		$( document ).on(
			'click',
			'.notice-mdjm-dismiss .notice-dismiss',
			function () {
		 ...
		 */
		 var MDJM_Events = {
			...
			equipment : function()	{
				...
				$( document.body ).on(
					'change',
					'#_mdjm_event_package,#event_addons,#_mdjm_event_additional_cost,#_mdjm_event_discount',
					function() {
						setCost();
					}
					);
				...
				$( document.body ).on(
					'change',
					'#_mdjm_event_dj,#mdjm_event_type,#display_event_date',
					function(event) {
						event.preventDefault();
						var current_deposit = $( '#_mdjm_event_deposit' ).val();
						var postData        = {
			...
			time : function()	{
				...
				// Check the end date or time isnt before start

				$( document.body ).on(
					'change',
					'#event_finish_hr, #event_finish_min, #event_finish_period, #display_event_finish_date',
					function() {
						var start_hr     = $( '#event_start_hr' ).val();
						var start_min   = $( '#event_start_min' ).val();
						var start_date_     = $( '#_mdjm_event_date' ).val().split("-");
						var start_date = new Date(start_date_[0], start_date_[1] - 1, start_date_[2]);
						start_date.setHours(start_hr);
						start_date.setMinutes(start_min);
						var event_start = start_date;

						var finish_hr     = $( '#event_finish_hr' ).val();
						var finish_min   = $( '#event_finish_min' ).val();
						var finish_date_     = $( '#_mdjm_event_end_date' ).val().split("-");
						var finish_date = new Date(finish_date_[0], finish_date_[1] - 1, finish_date_[2]);
						finish_date.setHours(finish_hr);
						finish_date.setMinutes(finish_min);
			...
			travel : function()	{
				...
				$( document.body ).on(
					'change',
					'#_mdjm_event_dj,#venue_address1,#venue_address2,#venue_town,#venue_county,#venue_postcode',
					function() {
						setTravelData();
		 ...
		 */
		 var MDJM_Equipment = {
		 	...
		 	clone_repeatable : function(row) {
				...
				var highest = 1;
				var key     = highest;
				row.parent().find( 'tr.mdjm_repeatable_row' ).each(
					function() {
				...
				clone.find( 'span.mdjm_price_id' ).each(
					function() {
						$( this ).text( parseInt( key ) );
					}
					);

				clone.find( 'span.mdjm_file_id' ).each(
					function() {
						$( this ).text( parseInt( key ) );
					}
					);

				clone.find( '.mdjm_repeatable_default_input' ).each(
					function() {
			...
			move : function() {
				...
				$( '.mdjm_repeatable_table tbody' ).sortable(
				{
					handle: '.mdjm_draghandle', items: '.mdjm_repeatable_row', opacity: 0.6, cursor: 'move', axis: 'y', update: function() {
						...
						$( this ).find( 'tr' ).each(
							function() {
								$( this ).find( 'input.mdjm_repeatable_index' ).each(
									function() {
										$( this ).val( count );
									}
									);
		 ...
		 */
		 var MDJM_Reports = {
		 	...
		 	date_options : function() {
				...
				$( '#mdjm-graphs-date-options' ).change(
					function() {
						var $this          = $( this ),
		 ...
		 var MDJM_Export = {
		 	...
		 	process_step : function( step, data, self ) {
				...
				dismiss_message : function() {
					$( 'body' ).on(
						'click',
						'#mdjm-batch-success .notice-dismiss',
						function() {
