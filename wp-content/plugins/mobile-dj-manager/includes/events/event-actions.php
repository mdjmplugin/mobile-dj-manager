...
/**
 * Accept an enquiry from the Client Portal.
 *
 * @since 1.3
...
function mdjm_event_add_journal_after_status_change( $result, $event_id, $new_status, $old_status, $args ) {
	...
	$reject_reason = ( 'mdjm-rejected' === $new_status && ! empty( $args['reject_reason'] ) ) ? '<br />' . $args['reject_reason'] : '';

	$comment_args = array(
		'user_id'         => is_user_logged_in() ? get_current_user_id() : 1,
		'event_id'        => $event_id,
		'comment_content' => sprintf(
			__( '%1$s status updated to %2$s.%3$s', 'mobile-dj-manager' ),
			esc_html( mdjm_get_label_singular() ),
			mdjm_get_event_status( $event_id ),
			$reject_reason
		),
	);
