...
function mdjm_run_install() {
	...
	if ( ! array_key_exists( 'app_home_page', $current_options ) ) {
		...
		$contract = wp_insert_post(
			array(
				'post_title'     => sprintf( esc_html__( '%s Contract', 'mobile-dj-manager' ), esc_html( mdjm_get_label_singular() ) ),
		...
		$quotes = wp_insert_post(
			array(
				'post_title'     => sprintf( esc_html__( '%s Quotes', 'mobile-dj-manager' ), esc_html( mdjm_get_label_singular() ) ),
	...
	if ( ! array_key_exists( 'enquiry', $current_options ) ) {
		...
		$enquiry = wp_insert_post(
			array(
				...
				'post_content'   => '<h1>' . __( 'Your DJ Enquiry from {company_name}', 'mobile-dj-manager' ) . '</h1>' .
									...
									'<br /><br />' .
									sprintf( esc_html__( 'Thank you for contacting {company_name} regarding your up and coming %s on {event_date}.', 'mobile-dj-manager' ), esc_html( mdjm_get_label_singular( true ) ) ) .
									'<br /><br />' .
		...
		$online_enquiry = wp_insert_post(
			array(
				...
				'post_content'   => sprintf( '[caption id="" align="alignleft" width="128"]%2$s[/caption]', '{website_url}', '{company_name}' ) .
									'<h3>' . sprintf( esc_html__( '%1$s Quotation for %2$s', 'mobile-dj-manager' ), esc_html( mdjm_get_label_singular() ), '{client_fullname}' ) . '</h3>' .
									'<pre>' . sprintf( esc_html__( 'Prepared by: %s', 'mobile-dj-manager' ), '{employee_fullname}' ) .
									'<br />' .
									__( 'Date:', 'mobile-dj-manager' ) . ' {DDMMYYYY}' .
									'<br />' .
									__( 'Valid for: 2 weeks from date', 'mobile-dj-manager' ) .
									'</pre><br />' .
									sprintf( esc_html__( 'Dear %s,', 'mobile-dj-manager' ), '{client_firstname}' ) .
									'<br />' .
									sprintf( esc_html__( 'It is with pleasure that I am providing you with the following costs for your %1$s on %2$s.', 'mobile-dj-manager' ), esc_html( mdjm_get_label_singular( true ) ), '{event_date}' ) .
									'<br /><br />' .
									...
									'<tbody>' .
									'<tr>' .
									'<td>' . sprintf( esc_html__( '%s Date:', 'mobile-dj-manager' ), esc_html( mdjm_get_label_singular() ) ) . '</td>' .
									'<td>{event_date}</td>' .
									'<td>' . sprintf( esc_html__( '%s Type:', 'mobile-dj-manager' ), esc_html( mdjm_get_label_singular() ) ) . '</td>' .
									'<td>{event_type}</td>' .
									'</tr>' .
									'<tr>' .
									...
									'<td colspan="2">' . sprintf( esc_html__( '%s Cost:', 'mobile-dj-manager' ), esc_html( mdjm_get_label_singular() ) ) . '</td>' .
		...
		$contract = wp_insert_post(
			array(
				...
				'ping_status'    => 'closed',
				'comment_status' => 'closed',
				'post_content'   => '<h2>' . sprintf( esc_html__( 'Your Booking with %s', 'mobile-dj-manager' ), '{company_name}' ) . '</h2>' .
									sprintf( esc_html__( 'Dear %s,', 'mobile-dj-manager' ), '{client_firstname}' ) .
									'<br /><br />' .
									sprintf( esc_html__( 'Thank you for indicating that you wish to proceed with booking %1$s for your up and coming %2$s on %3$s', 'mobile-dj-manager' ), '{company_name}', esc_html( mdjm_get_label_singular( true ) ), '{event_date}' ) .
									'<br /><br />' .
									...
									sprintf( __( 'Your deposit of <strong>%1$s</strong> is now due. If you have not already done so please make this payment now. Details of how to make this payment are shown within the <a href="%2$s">contract</a>', 'mobile-dj-manager' ), '{deposit}', '{contract_url}' ) . '</li>' .
									'</ul><br />' .
									...
									'<br /><br />' .
									sprintf( esc_html__( 'Thank you for choosing %s', 'mobile-dj-manager' ), '{company_name}' ) .
									'<br /><br />' .
		...
		$booking_conf_client = wp_insert_post(
			array(
				...
				'post_content'   => '<h1>' . __( 'Booking Confirmation', 'mobile-dj-manager' ) . '</h1>' .
									...
									'<br /><br />' .
									sprintf( __( 'Thank you for booking your up and coming %1$s with %2$s. Your booking is now confirmed.', 'mobile-dj-manager' ), esc_html( mdjm_get_label_singular() ), '{company_name}' ) .
									'<br /><br />' .
									sprintf( __( 'My name is %1$s and I will be your DJ on %2$s. Should you wish to contact me at any stage to discuss your %3$s, my details are at the end of this email.', 'mobile-dj-manager' ), '{employee_fullname}', '{event_date}', esc_html( mdjm_get_label_singular( true ) ) ) .
									'<br />' .
									'<h2>' . __( 'What Now?', 'mobile-dj-manager' ) . '</h2>' .
									'<br />' .
									'<strong>' . __( 'Music Selection & Playlists', 'mobile-dj-manager' ) . '</strong>' .
									'<br /><br />' .
									sprintf( __( 'We have an online portal where you can add songs that you would like to ensure we play during your disco. To access this feature, head over to the %1$s <a href="%2$s">%2$s</a>. The playlist feature will close %3$s days before your %4$s.', 'mobile-dj-manager' ), '{company_name}', '{application_home}', '{playlist_close}', esc_html( mdjm_get_label_singular( true ) ) ) .
									'<br /><br />' .
									...
									'<br /><br />' .
									sprintf( __( 'I generally contact you again approximately 2 weeks before your %s to finalise details with you. However, if you have any questions, concerns, or just want a general chat about your disco, feel free to contact me at any time.', 'mobile-dj-manager' ), esc_html( mdjm_get_label_singular( true ) ) ) .
									'<br /><br />' .
									sprintf( __( 'Thanks again for choosing %1$s to provide the DJ & Disco for your %2$s. I look forward to partying with you on %3$s.', 'mobile-dj-manager' ), '{company_name}', esc_html( mdjm_get_label_singular( true ) ), '{event_date}' ) .
									'<br /><br />' .
									__( 'Best Regards', 'mobile-dj-manager' ) .
									'<br /><br />' .
									'{employee_fullname}' .
		...
		$email_dj_confirm = wp_insert_post(
			array(
				...
				'comment_status' => 'closed',
				'post_content'   => '<h1>' . __( 'Booking Confirmation', 'mobile-dj-manager' ) . '</h1>' .
									sprintf( esc_html__( 'Dear %s,', 'mobile-dj-manager' ), '{employee_firstname}' ) .
									'<br /><br />' .
									sprintf( esc_html__( 'Your client %1$s has just confirmed their booking for you to DJ at their %2$s on %3$s.', 'mobile-dj-manager' ), '{client_fullname}', esc_html( mdjm_get_label_singular( true ) ), '{event_date}' ) .
									'<br /><br />' .
									sprintf( esc_html__( 'A booking confirmation email has been sent to them and they now have your contact details and access to the online %s tools to create playlist entries etc.', 'mobile-dj-manager' ), '{application_name}' ) .
									'<br /><br />' .
									sprintf( __( 'Make sure you login regularly to the <a href="%1$s">%2$s %3$s admin interface</a> to ensure you have all relevant information relating to their booking.', 'mobile-dj-manager' ), admin_url(), '{company_name}', '{application_name}' ) .
									'<br /><br />' .
									sprintf( esc_html__( 'Remdjmber it is your responsibility to remain in regular contact with your client regarding their %1$s as well as answer any queries or concerns they may have. Customer service is one of our key selling points and after the event, your client will be invited to provide feedback regarding the booking process, communication in the lead up to the %1$s, as well as the %1$s itself.', 'mobile-dj-manager' ), esc_html( mdjm_get_label_singular( true ) ) ) .
									'<br />' .
									'<h2>' . sprintf( esc_html__( '%s Details', 'mobile-dj-manager' ), esc_html( mdjm_get_label_singular() ) ) . '</h2>' .
									'<br />' .
									...
									'<br />' .
									sprintf( __( 'Further information is available on the <a href="%1$s">%2$s %3$s admin interface</a>.', 'mobile-dj-manager' ), admin_url(), '{company_name}', '{application_home}' ) .
									'<br /><br />' .
									__( 'Regards', 'mobile-dj-manager' ) .
									'<br /><br />' .
									'{company_name}',
		...
		$unavailable = wp_insert_post(
			array(
				'post_title'     => sprintf( esc_html__( '%s is not Available', 'mobile-dj-manager' ), '{company_name}' ),
				'post_status'    => 'publish',
				'post_type'      => 'email_template',
				'post_author'    => 1,
				'ping_status'    => 'closed',
				'comment_status' => 'closed',
				'post_content'   => '<h1>' . sprintf( esc_html__( 'Your DJ Enquiry with %s', 'mobile-dj-manager' ), '{company_name}' ) . '</h1>' .
									sprintf( esc_html__( 'Dear %s', 'mobile-dj-manager' ), '{client_firstname}' ) .
									'<br /><br />' .
									sprintf( esc_html__( 'Thank you for contacting %1$s regarding your up and coming %2$s on %3$s.', 'mobile-dj-manager' ), '{company_name}', esc_html( mdjm_get_label_singular( true ) ), '{event_date}' ) .
									'<br /><br />' .
									sprintf( esc_html__( 'Unfortunately however, we are not available on the date you have selected for your %s.', 'mobile-dj-manager' ), esc_html( mdjm_get_label_singular( true ) ) ) .
									__( "If you have alternative dates you are looking at, we'd love to hear from you again.", 'mobile-dj-manager' ) .
									'<br /><br />' .
									sprintf( esc_html__( 'Otherwise, we hope you have a great %s and hope to hear from you again next time.', 'mobile-dj-manager' ), esc_html( mdjm_get_label_singular( true ) ) ) .
									'<br /><br />' .
									__( 'Best Regards', 'mobile-dj-manager' ) .
									'<br /><br />' .
									'{employee_fullname}' .
		...
		$payment_cfm = wp_insert_post(
			array(
				'post_title'     => sprintf( esc_html__( '%1$s %2$s Payment Confirmation', 'mobile-dj-manager' ), esc_html( mdjm_get_label_singular() ), '{payment_for}' ),
				'post_status'    => 'publish',
				'post_type'      => 'email_template',
				'post_author'    => 1,
				'ping_status'    => 'closed',
				'comment_status' => 'closed',
				'post_content'   => '<h4><span style="color: #ff9900;">' . sprintf( esc_html__( 'Thank you for your %s payment', 'mobile-dj-manager' ), '{payment_for}' ) . '</span></h4>' .
									sprintf( esc_html__( 'Dear %s,', 'mobile-dj-manager' ), '{client_firstname}' ) .
									'<br /><br />' .
									sprintf( __( 'Thank you for your recent payment of <strong>%1$s</strong> towards the <strong>%2$s</strong> for you event on <strong>%3$s</strong>. Your payment has been received and your %4$s details have been updated.', 'mobile-dj-manager' ), '{payment_amount}', '{payment_for}', '{event_date}', esc_html( mdjm_get_label_singular( true ) ) ) .
									'<br /><br />' .
									...
									'<br /><br />' .
									sprintf( __( "Your username is %1\$s and if you can't recall your password, you can reset it by clicking the <a title='Reset your password for the %2\$s %3\$s' href='%4\$s'>Lost Password</a> link.", 'mobile-dj-manager' ), '{client_username}', '{company_name}', '{application_name}', wp_lostpassword_url() ) .
									'<br /><br />' .
									__( 'Best Regards', 'mobile-dj-manager' ) .
									'<br /><br />' .
									'{employee_fullname}' .
		...
		$options['email_invoice']               = $email_invoice;
	...
	foreach ( mdjm_get_registered_settings() as $tab => $sections ) {
		...
		if ( is_array( $tab_sections )) {
			foreach( $tab_sections as $tsection){
				if ( ! property_exists( $tsection, $section)){
					$section  = 'main';
					$settings = $sections;			
				}
			...
			if ( ! property_exists( $tab_sections, $section)){
				$section  = 'main';
				$settings = $sections;			
			}
		}
	...
	wp_insert_term(
		...
		'transaction-types',
		array(
			'description' => __( 'Term used for payments that are a contribution towards balance', 'mobile-dj-manager' ),
			'slug'        => 'mdjm-other-amount',
		)
	);
	...
	wp_insert_term( __( 'Dry Ice Allowed', 'mobile-dj-manager' ), 'venue-details', array( 'description' => __( 'Venue allows the use of Dry Ice', 'mobile-dj-manager' ) ) );
	...
	wp_insert_term( __( 'Lift on site', 'mobile-dj-manager' ), 'venue-details', array( 'description' => __( 'Access to this Venue is via a lift', 'mobile-dj-manager' ) ) );
