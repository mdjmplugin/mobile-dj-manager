...
function mdjm_get_availability_page_action_links() {
	...
	// $actions['availability_check'] = '<a id="mdjm-check-availabilty-action" href="#" class="toggle-availability-checker-section">' . __( 'Show availability checker', 'mobile-dj-manager' ) . '</a>';
	...
	<div class="mdjm-availability-checker-fields">
		...
		<?php if ( mdjm_employee_can( 'manage_employees' ) ) : ?>
			<div class="mdjm-event-primary-employee">
				<span class="mdjm-repeatable-row-setting-label">
					<?php printf( '%s', $artist ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?> 
				</span>
				<?php
			...
			<div class="mdjm-custom-event-section">
					...
					<span class="mdjm-employee-option">
						...
						echo MDJM()->html->employee_dropdown( // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
							array(
								...
								'selected' => $employee_id, // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
								'group'    => mdjm_is_employer(), // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
							'chosen'       => true,
							)
						);
				...
				<?php else : ?>
					...
					echo MDJM()->html->hidden( // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
						array(
							'name'  => 'absence_employee_id',
							'value' => $employee_id, // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
						)
					);
					?>
				<?php endif; ?>
