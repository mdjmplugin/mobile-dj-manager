...
function mdjm_comms_email_contextual_help() {
	...
	if ( 'mdjm-event_page_mdjm-comms' !== $screen->id ) {
		return;
	}
	...
	$screen->add_help_tab(
		array(
			...
			'content' =>
			'<p>' . sprintf( /* translators: %1$s Staff Type */
				__( '<strong>Select a Recipient</strong> - Choose from the dropdown list who your email is to. Users are grouped into Clients and Employees. Once you have selected a recipient the Associated %1$s list will be updated with their active %2$s. This is a required field.', 'mobile-dj-manager' ),
			...
			'<p>' . sprintf( /* translators: %s Event Staff type %1 Staff $s plural*/
