...
function mdjm_sign_event_contract( $event_id, $details ) {
	...
	$contract_signatory_content .= '<p style="font-weight: bold">' . __( 'IP Address Used', 'mobile-dj-manager' ) . ': ' . isset( $_SERVER['REMOTE_ADDR'] ) ? esc_html( sanitize_text_field( wp_unslash( $_SERVER['REMOTE_ADDR'] ) ) ) : 'N/A' . '</p>' . "\r\n";
	...
	$event_meta = array(
		...
		'_mdjm_event_contract_approver'    => wp_strip_all_tags( wp_slash( ucfirst( $details['mdjm_first_name'] ) . ' ' . ucfirst( $details['mdjm_last_name'] ) ) ),
		'_mdjm_event_contract_approver_ip' => isset( $_SERVER['REMOTE_ADDR'] ) ? esc_html( sanitize_text_field( wp_unslash( $_SERVER['REMOTE_ADDR'] ) ) ) : 'N/A',
	...
	if ( mdjm_require_deposit_before_confirming() && ( 'Paid' != $event->get_deposit_status() ) ) {
