...
class MDJM_MC_API {
	...
	public function __construct() {
		$this->api_key = mdjm_get_option( 'mc_api_key' );

		$dash_position = strpos( $this->api_key, '-' );

		if ( $dash_position !== false ) {
			$this->api_url = 'https://' . substr( $this->api_key, $dash_position + 1 ) . '.api.mailchimp.com/2.0/';
		}
	}
	...
	private function show_error( $message ) {
		...
		add_settings_error( 'mdjm-mc-api', 'mdjm-mc-api-error', $message, 'error' );
		return true;
	}
	 ...
	 * @param   arr  $merge_vars         Array of extra merge variables
	 * @param   bool $replace_interests  Replace interest groupings, only if update_existing is true.
	 *
	 * @return  bool|str    True if success, 'error' if error
	 */
	public function subscribe( $email, $merge_vars = array(), $replace_interests = true ) {
		...
		if ( $list_id ) {
			...
			$data = array(
				...
				'email_type'        => mdjm_get_option( 'mc_format', 'html' ),
				'double_optin'      => mdjm_get_option( 'mc_double_optin', false ),
				'update_existing'   => mdjm_get_option( 'mc_update', false ),
				'replace_interests' => $replace_interests,
				'send_welcome'      => mdjm_get_option( 'mc_welcome', false ),
			);

			$data = apply_filters( 'mdjm_mc_subscribe_data', $data );
	 ...
	 *
	 * @since   1.0
	 * @param   arr $list_ids   Array of ID's of the lists to fetch.
	 * @return  bool
	 */
	...
	public function call( $method, array $data = array() ) {
		...
		$request_args = array(
			...
			'sslverify' => apply_filters( 'mdjm_mc_subscribe_use_sslverify', true ),
		);
