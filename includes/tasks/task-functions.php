...
function mdjm_get_task_run_times( $id = false ) {
	$event_label = esc_html( mdjm_get_label_singular() );
	$run_times   = array(
		'event_created'  => sprintf( esc_html__( 'After the %s is Created', 'mobile-dj-manager' ), $event_label ),
		'after_approval' => sprintf( esc_html__( 'After the %s is Confirmed', 'mobile-dj-manager' ), $event_label ),
		'before_event'   => sprintf( esc_html__( 'Before the %s', 'mobile-dj-manager' ), $event_label ),
		'after_event'    => sprintf( esc_html__( 'After the %s', 'mobile-dj-manager' ), $event_label ),
	);
...
function mdjm_request_deposit_single_task( $event_id ) {
	...
	if ( ! empty( $task['options']['email_template'] ) && ! empty( $event->client ) ) {
		...
		$email_args = array(
			...
			'track'     => true,
			'source'    => sprintf( esc_html__( 'Request %s Scheduled Task', 'mobile-dj-manager' ), mdjm_get_deposit_label() ),
		);
		...
		if ( mdjm_send_email_content( $email_args ) ) {
			...
			mdjm_add_journal(
				array(
					'user_id'         => 1,
					'event_id'        => $event->ID,
					'comment_content' => sprintf( esc_html__( '%s task manually executed', 'mobile-dj-manager' ), esc_attr( $task['name'] ) ) . '<br /><br />' . time(),
				)
			);
...
function mdjm_balance_reminder_single_task( $event_id ) {
	...
	if ( ! empty( $task['options']['email_template'] ) && ! empty( $event->client ) ) {
		...
		$email_args = array(
			...
			'message'   => mdjm_get_email_template_content( $task['options']['email_template'] ),
			'track'     => true,
			'source'    => sprintf( esc_html__( 'Request %s Scheduled Task', 'mobile-dj-manager' ), mdjm_get_balance_label() ),
		);
		...
		if ( mdjm_send_email_content( $email_args ) ) {
			...
			mdjm_add_journal(
				array(
					'user_id'         => 1,
					'event_id'        => $event->ID,
					'comment_content' => sprintf( esc_html__( '%s task manually executed', 'mobile-dj-manager' ), $task['name'] ) . '<br /><br />' . time(),
				)
			);
...
function mdjm_employee_playlist_notify_single_task( $event_id ) {
	...
	$args = array(
		...
		'event_id'   => $event_id,
		'client_id'  => $event->client,
		'subject'    => sprintf( esc_html__( 'Playlist for %1$s ID %2$s', 'mobile-dj-manager' ), esc_html( mdjm_get_label_singular() ), '{contract_id}' ),
