...
function mdjm_run_install() {
	...
	if ( ! array_key_exists( 'enquiry', $current_options ) ) {
		...
		$online_enquiry = wp_insert_post(
			array(
				...
				'post_content'   => sprintf( '[caption id="" align="alignleft" width="128"]<a href="%1$s"><img title="%2$s" src="http://www.mydjplanner.co.uk/wp-content/uploads/2014/10/icon-128x1281.png" alt="%2$s" width="128" height="128" /></a> %2$s[/caption]', '{website_url}', '{company_name}' ) .
									'<h3>' . sprintf( __( '%1$s Quotation for %2$s', 'mobile-dj-manager' ), mdjm_get_label_singular(), '{client_fullname}' ) . '</h3>' .
									...
									'<br />' .
									sprintf( __( 'It is with pleasure that I am providing you with the following costs for your %1$s on %2$s.', 'mobile-dj-manager' ), mdjm_get_label_singular( true ), '{event_date}' ) .
									'<br /><br />' .
		...
		$contract = wp_insert_post(
			array(
				...
				'post_content'   => '<h2>' . sprintf( __( 'Your Booking with %s', 'mobile-dj-manager' ), '{company_name}' ) . '</h2>' .
									sprintf( __( 'Dear %s,', 'mobile-dj-manager' ), '{client_firstname}' ) .
									'<br /><br />' .
									sprintf( __( 'Thank you for indicating that you wish to proceed with booking %1$s for your up and coming %2$s on %3$s', 'mobile-dj-manager' ), '{company_name}', mdjm_get_label_singular( true ), '{event_date}' ) .
									'<br /><br />' .
		...
		$booking_conf_client = wp_insert_post(
			array(
				...
				'post_content'   => '<h1>' . __( 'Booking Confirmation', 'mobile-dj-manager' ) . '</h1>' .
									sprintf( __( 'Dear %s,', 'mobile-dj-manager' ), '{client_firstname}' ) .
									'<br /><br />' .
									sprintf( __( 'Thank you for booking your up and coming %1$s with %2$s. Your booking is now confirmed.', 'mobile-dj-manager' ), mdjm_get_label_singular(), '{company_name}' ) .
									'<br /><br />' .
									sprintf( __( 'My name is %1$s and I will be your DJ on %2$s. Should you wish to contact me at any stage to discuss your %3$s, my details are at the end of this email.', 'mobile-dj-manager' ), '{employee_fullname}', '{event_date}', mdjm_get_label_singular( true ) ) .
									'<br />' .
									...
									'<br /><br />' .
									sprintf( __( 'Thanks again for choosing %1$s to provide the DJ & Disco for your %2$s. I look forward to partying with you on %3$s.', 'mobile-dj-manager' ), '{company_name}', mdjm_get_label_singular( true ), '{event_date}' ) .
									'<br /><br />' .
									__( 'Best Regards', 'mobile-dj-manager' ) .
									'<br /><br />' .
									'{employee_fullname}' .
		...
		$email_dj_confirm = wp_insert_post(
			array(
				...
				'post_content'   => '<h1>' . __( 'Booking Confirmation', 'mobile-dj-manager' ) . '</h1>' .
									...
									'<br /><br />' .
									sprintf( __( 'Your client %1$s has just confirmed their booking for you to DJ at their %2$s on %3$s.', 'mobile-dj-manager' ), '{client_fullname}', mdjm_get_label_singular( true ), '{event_date}' ) .
									'<br /><br />' .
		...
		$unavailable = wp_insert_post(
			array(
				...
				'post_content'   => '<h1>' . sprintf( __( 'Your DJ Enquiry with %s', 'mobile-dj-manager' ), '{company_name}' ) . '</h1>' .
									sprintf( __( 'Dear %s', 'mobile-dj-manager' ), '{client_firstname}' ) .
									'<br /><br />' .
									sprintf( __( 'Thank you for contacting %1$s regarding your up and coming %2$s on %3$s.', 'mobile-dj-manager' ), '{company_name}', mdjm_get_label_singular( true ), '{event_date}' ) .
									'<br /><br />' .
		...
		$payment_cfm = wp_insert_post(
			array(
				'post_title'     => sprintf( __( '%1$s %2$s Payment Confirmation', 'mobile-dj-manager' ), mdjm_get_label_singular(), '{payment_for}' ),
				'post_status'    => 'publish',
				'post_type'      => 'email_template',
				'post_author'    => 1,
				'ping_status'    => 'closed',
				...
				'post_content'   => '<h4><span style="color: #ff9900;">' . sprintf( __( 'Thank you for your %s payment', 'mobile-dj-manager' ), '{payment_for}' ) . '</span></h4>' .
									sprintf( __( 'Dear %s,', 'mobile-dj-manager' ), '{client_firstname}' ) .
									'<br /><br />' .
									sprintf( __( 'Thank you for your recent payment of <strong>%1$s</strong> towards the <strong>%2$s</strong> for you event on <strong>%3$s</strong>. Your payment has been received and your %4$s details have been updated.', 'mobile-dj-manager' ), '{payment_amount}', '{payment_for}', '{event_date}', mdjm_get_label_singular( true ) ) .
									'<br /><br />' .
	...
	// Transaction Terms
	wp_insert_term( __( 'Deposit', 'mobile-dj-manager' ), 'transaction-types', array(
        'description' => __( 'Event deposit payments are assigned to this term', 'mobile-dj-manager' ),
        'slug'        => 'mdjm-deposit-payments',
	) );
	wp_insert_term( __( 'Balance', 'mobile-dj-manager' ), 'transaction-types', array(
	...
	wp_insert_term( __( 'Employee Wages', 'mobile-dj-manager' ), 'transaction-types', array(
	...
	wp_insert_term( __( 'Merchant Fees', 'mobile-dj-manager' ), 'transaction-types', array(
	...
	wp_insert_term( __( 'Other Amount', 'mobile-dj-manager' ), 'transaction-types', array(
	...
	if ( ! $current_version ) {
		...
		foreach ( $upgrade_routines as $upgrade ) {
			mdjm_set_upgrade_complete( $upgrade );
		}
	}
