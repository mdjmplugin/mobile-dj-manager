...
function mdjm_events_contextual_help() {
	...
	$screen->set_help_sidebar(
		...
		'<p>' . sprintf(
			...
			esc_url( 'https://www.facebook.com/groups/mdjmusers/' )
		) . '</p>' .
		'<p>' . sprintf(
			/* translators: %s URL for section */
	...
	$screen->add_help_tab(
		array(
			...
			/* translators: %s Event/Events Options */
			'title'   => sprintf( __( '%s Options', 'mobile-dj-manager' ), $singular ),
			'content' =>
			'<p>' . sprintf( /* translators: %1 %2 Event %3 URL */
	...
	$screen->add_help_tab(
		array(
			...
			'content' =>
			'<p>' . sprintf(
				/* translators: %1 %2 Event label %3 URL */
				__( '<strong>%1$s Type</strong> - Set the type of %2$s <em>i.e. Wedding or 40th Birthday</em>. You can define the types <a href="%3$s">here</a> or simply click the <i class="fas fa-plus"></i> icon to add a new %2$s type inline.', 'mobile-dj-manager' ),
				$singular,
				strtolower( $singular ),
				admin_url( 'edit-tags.php?taxonomy=event-types&post_type=mdjm-event' )
			) . '</p>' .
			/* translators: %1 %2 Event label */
			...
			'<p>' . sprintf(
				/* translators: %1 Deposit label %2 Contract label */
