...
function mdjm_venue_posts_custom_column( $column_name, $post_id )	{
	...
	switch ( $column_name ) {
		case 'contact':
			echo sprintf(
				'<a href="mailto:%s">%s</a>',
				esc_html( get_post_meta( $post_id, '_venue_email', true ) ),
				esc_html( wp_unslash( get_post_meta( $post_id, '_venue_contact', true ) ) )
			);
			break;

		// Phone
		case 'phone':
			echo esc_html( get_post_meta( $post_id, '_venue_phone', true ) );
			break;

		// Town
		case 'town':
			echo esc_html( get_post_meta( $post_id, '_venue_town', true ) );
			break;

		// County
		case 'county':
			echo esc_html( get_post_meta( $post_id, '_venue_county', true ) );
			break;
		...
		// Information
		case 'info':
			echo esc_html( wp_unslash( get_post_meta( $post_id, '_venue_information', true ) ) );
			break;
