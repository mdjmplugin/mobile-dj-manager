			...
			array(
				'main'      => array(
					...
					'show_credits'        => array(
						...
						'desc' => sprintf( __( 'Whether or not to display the %1$sPowered by %2$s, version %3$s%4$s text at the footer of the %5$s application pages.', 'mobile-dj-manager' ), '<span class="mdjm-admin-footer">', MDJM_NAME, MDJM_VERSION_NUM, '</span>', mdjm_get_application_name() ),
						'type' => 'checkbox',
					),
				...
				'debugging' => array(
					...
					'debug_log_size'     => array(
						...
						'desc' => __( 'The max size in Megabytes to allow your log files to grow to before you receive a warning (if configured below)', 'mobile-dj-manager' ),
						'type' => 'text',
						'size' => 'small',
						'std'  => '2',
					),
			...
			array(
				...
				'playlist' => array(
					...
					'close'             => array(
						...
						'hint' => sprintf( __( 'Enter %1$s0%2$s to never close.', 'mobile-dj-manager' ),
							'<code>',
							'</code>'
						),
			...
			array(
				'main'      => array(
					...
					'bcc_dj_to_client'              => array(
						...
						'desc' => sprintf( __( 'Send a copy of client emails to the %1$s primary %2$s', 'mobile-dj-manager' ),
					...
					'bcc_admin_to_client'           => array(
						...
						'desc' => sprintf( __( 'Send a copy of client emails to %1$sDefault From Address%2$s', 'mobile-dj-manager' ),
							'<code>',
							'</code>'
						),
						'type' => 'checkbox',
				...
				'templates' => array(
					...
					'online_enquiry'         => array(
						...
						'desc'    => sprintf( __( 'This is the default template used for clients viewing quotes online via the %s.', 'mobile-dj-manager' ), mdjm_get_application_name() ),
						'type'    => 'select',
                        'chosen'  => true,
					...
					'booking_conf_to_dj'     => array(
						...
						'desc' => sprintf( __( 'Email %1$s primary %2$s with selected template when booking is confirmed i.e. contract accepted, or status changed to Approved', 'mobile-dj-manager' ), mdjm_get_label_plural( true ), mdjm_get_option( 'artist', __( 'DJ', 'mobile-dj-manager' ) ) ),
						'type' => 'checkbox',
					),
			...
			array(
				'main'         => array(
					...
					'app_name'                   => array(
						...
						'hint' => sprintf( __( 'Default is %1$sClient Zone%2$s.', 'mobile-dj-manager' ), '<code>', '</code>' ),
					...
					'package_prices'             => array(
						...
						'desc' => sprintf( __( 'Select to display %1$s package &amp; Add-on prices within hover text within the %2$s', 'mobile-dj-manager' ), mdjm_get_label_singular( true ), mdjm_get_application_name() ),
						'type' => 'checkbox',
					),
				),
				'styles'       => array(
				...
				'pages'        => array(
					...
					'app_home_page'  => array(
						...
						'desc'    => sprintf( __( 'Select the home page for the %1$s application. Needs to contain the shortcode %2$s[mdjm-home]%3$s', 'mobile-dj-manager' ), mdjm_get_application_name(), '<code>', '</code>' ),
						'type'    => 'select',
					...
					'contracts_page' => array(
						...
						'desc'    => sprintf( __( "Select your website's contracts page. Needs to contain the shortcode %1\$s[mdjm-contract]%2\$s", 'mobile-dj-manager' ), '<code>', '</code>' ),
						'type'    => 'select',
						'options' => mdjm_list_pages(),
					),
					'payments_page'  => array(
						...
						'desc'    => sprintf( __( "Select your website's payments page. Needs to contain the shortcode %1\$s[mdjm-payments]%2\$s", 'mobile-dj-manager' ), '<code>', '</code>' ),
						'type'    => 'select',
						'options' => mdjm_list_pages(),
					),
					'playlist_page'  => array(
						...
						'desc'    => sprintf( __( "Select your website's playlist page. Needs to contain the shortcode %1\$s[mdjm-playlist]%2\$s", 'mobile-dj-manager' ), '<code>', '</code>' ),
						'type'    => 'select',
						'options' => mdjm_list_pages(),
					),
					'profile_page'   => array(
						...
						'desc'    => sprintf( __( "Select your website's profile page. Needs to contain the shortcode %1\$s[mdjm-profile]%2\$s", 'mobile-dj-manager' ), '<code>', '</code>' ),
						'type'    => 'select',
						'options' => mdjm_list_pages(),
					),
				),
			...
			array(
				'main'                  => array(
					...
					'deposit_type'           => array(
						...
						'desc'    => sprintf( __( 'If you require ' . mdjm_get_deposit_label() . ' payments for your %s, how should they be calculated?', 'mobile-dj-manager' ), mdjm_get_label_plural( true ) ),
						'type'    => 'select',
						'options' => array(
							'0'          => 'Not required',
					...
					'deposit_amount'         => array(
						...
						'desc' => sprintf( __( "If your %1\$s's are a percentage enter the value (i.e 20). For fixed prices, enter the amount in the format %2\$s", 'mobile-dj-manager' ), mdjm_get_deposit_label(), mdjm_format_amount( '0' ) ),
						'type' => 'text',
						'size' => 'small',
					),
					'deposit_before_confirm' => array(
						...
						'desc' => sprintf( __( 'Wait for %s to be paid before the booking confirmation is issued', 'mobile-dj-manager' ), mdjm_get_deposit_label() ),
						'type' => 'checkbox',
					),
					...
					'other_amount_default'   => array(
						...
						'desc' => sprintf( __( 'Enter the default amount to be used in the %s field.', 'mobile-dj-manager' ), mdjm_get_option( 'other_amount_label', __( 'Other Amount', 'mobile-dj-manager' ) ) ),
						'type' => 'text',
						'size' => 'small',
						'std'  => '50.00',
					),
					...
					'default_type'           => array(
						...
						'desc'    => sprintf( __( 'What is the default method of payment? i.e. if you select an %1$s %2$s as paid how should we log it?', 'mobile-dj-manager' ), mdjm_get_label_singular( true ), mdjm_get_balance_label() ),
						'type'    => 'select',
			...
			array(
				...
				'terms_conditions' => array(
					...
					'agree_terms_heading'     => array(
						...
						'type' => 'text',
						'size' => 'regular',
						'std'  => sprintf( __( 'Terms and Conditions for %s', 'mobile-dj-manager' ), mdjm_get_label_plural() ),
					),
...
function mdjm_rich_editor_callback( $args ) {
	...
	if ( $wp_version >= 3.3 && function_exists( 'wp_editor' ) ) {
		ob_start();
		wp_editor( stripslashes( $value ), 'mdjm_settings_' . $args['id'], array(
