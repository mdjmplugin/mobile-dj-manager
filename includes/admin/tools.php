...
function mdjm_tools_page() {
	...
	$active_tab = isset( $_GET['tab'] ) ? sanitize_text_field( wp_unslash( $_GET['tab'] ) ) : 'system_info';

	?>
	<div class="wrap">
		<h1><?php esc_html_e( 'Mobile DJ Manager Tools', 'mobile-dj-manager' ); ?></h1>
...
function mdjm_tools_sysinfo_get() {
	...
	$return .= 'Memory Limit:             ' . WP_MDJMORY_LIMIT . "\n";
	...
	$email_invoice_template  = mdjm_get_option( 'email_invoice', '' );
	...
	$return .= 'Email Invoice:           ' . ( ! empty( $email_invoice_template ) ? get_the_title( $email_invoice_template ) . ' (' . $email_invoice_template . ')' . "\n" : "Unset\n" );
...
function mdjm_tools_import_export_display() {
	...
	<div class="postbox">
		...
		<div class="inside">
			<p><?php esc_html_e( 'Export the Mobile DJ Manager settings for this site as a .json file. This allows you to easily import the configuration into another site.', 'mobile-dj-manager' ); ?></p>
			<p>
			<?php
			printf(
				__( 'To export %1$s data (%2$s, clients, etc), visit the <a href="%3$s">Reports</a> page.', 'mobile-dj-manager' ), // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
	...
	<div class="postbox">
		...
		<div class="inside">
			<p><?php esc_html_e( 'Import the Mobile DJ Manager settings from a .json file. This file can be obtained by exporting the settings on another site using the form above.', 'mobile-dj-manager' ); ?></p>
...
function mdjm_tools_import_export_process_export() {
	...
	if ( ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['mdjm_export_nonce'], 'mdjm_export_nonce' ) ) ) ) {
		return;
	}
...
function mdjm_tools_import_export_process_import() {
	...
	if ( ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['mdjm_import_nonce'], 'mdjm_import_nonce' ) ) ) ) {
		return;
	}
