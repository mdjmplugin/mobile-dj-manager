...
function mdjm_admin_menu() {
	...
	global $mdjm_settings_page, $mdjm_contract_template_page, $mdjm_email_template_page,
		   ...
		   $mdjm_custom_client_fields_page, $mdjm_extensions_page, $mdjm_dashboard_page;
	...
	if ( mdjm_employee_can( 'send_comms' ) ) {
		...
		$mdjm_comms_history_page = add_submenu_page(
			'edit.php?post_type=mdjm-event',
			__( 'Communication History', 'mobile-dj-manager' ),
			__( 'Communication History', 'mobile-dj-manager' ),
			'mdjm_employee',
			'edit.php?post_type=mdjm_communication'
		);
	}
	...
	if ( ( mdjm_get_option( 'enable_packages' ) ) && mdjm_employee_can( 'manage_packages' ) ) {
		...
		$mdjm_addons_page   = add_submenu_page(
			'edit.php?post_type=mdjm-event',
			__( 'Addons', 'mobile-dj-manager' ),
			__( 'Addons', 'mobile-dj-manager' ),
			'mdjm_package_edit_own',
			'edit.php?post_type=mdjm-addon',
			''
		);
	}
	...
	$mdjm_settings_page = add_submenu_page( 'edit.php?post_type=mdjm-event', __( 'Settings', 'mobile-dj-manager' ), __( '<strong><span style="color:#F90">Settings</span></strong>', 'mobile-dj-manager' ), 'manage_mdjm', 'mdjm-settings', 'mdjm_options_page' );

	$mdjm_extensions_page = add_submenu_page( 'edit.php?post_type=mdjm-event', __( 'MDJM Extensions', 'mobile-dj-manager' ), __( '<strong><span style="color:#F90">Extensions</span></strong>', 'mobile-dj-manager' ), 'mdjm_employee', 'mdjm-addons', 'mdjm_extensions_page' );
	
	$mdjm_dashboard_page = add_submenu_page( 'edit.php?post_type=mdjm-event', __( 'Dashboard', 'mobile-dj-manager' ), __( '<strong><span style="color:#FFF">Dashboard</span></strong>', 'mobile-dj-manager' ), 'mdjm_employee', 'mdjm-dashboard', 'mdjm_dashboard_page' );
...
function mdjm_admin_toolbar( $admin_bar ) {
	...
	if ( mdjm_is_admin() ) {
		...
		$admin_bar->add_menu(
			array(
				...
				'title'  => __( 'Calendar', 'mobile-dj-manager' ),
	...
	$admin_bar->add_menu(
		array(
			'id'     => 'mdjm-user-guides',
			'parent' => 'mdjm',
			'title'  => sprintf( __( '%1$sDocumentation%2$s', 'mobile-dj-manager' ), '<span style="color:#F90">', '</span>' ),
	...
	$admin_bar->add_menu(
		array(
			'id'     => 'mdjm-support',
			'parent' => 'mdjm',
			'title'  => sprintf( __( '%1$sSupport%2$s', 'mobile-dj-manager' ), '<span style="color:#F90">', '</span>' ),
			'href'   => 'http://www.mdjm.co.uk/forums/',
			'meta'   => array(
