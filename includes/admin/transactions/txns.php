...
function mdjm_transaction_post_filtered( $query ) {
	...
	if ( ! empty( $_GET['mdjm_filter_type'] ) ) {
		...
		$type = isset( $_GET['mdjm_filter_type'] ) ? sanitize_text_field( wp_unslash( $_GET['mdjm_filter_type'] ) ) : 0;
	...
	switch ( $column_name ) {
		...
		case 'detail':
			...
			if ( is_array( $trans_types ) ) {
				...
				echo implode( '<br/>', $trans_types ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			}
			break;

		// Date
		case 'txn_date':
			echo esc_html( get_post_time( 'd M Y' ) );
			break;
		...
		case 'direction':
			if ( 'mdjm-income' == get_post_status( $post_id ) ) {
				...
				echo '<span style="color:green">' . esc_html__( 'In', 'mobile-dj-manager' ) . '</span>';

			} else {

				echo '<span style="color:red">&nbsp;&nbsp;&nbsp;&nbsp;' . esc_html__( 'Out', 'mobile-dj-manager' ) . '</span>';
		...
		// Source
		case 'payee':
			echo esc_html( mdjm_get_txn_recipient_name( $post_id ) );
		...
		case 'event':
			...
			if ( ! empty( $parent ) ) {
				...
				printf( '<a href="%s">%s</a>',
					...
					esc_html( mdjm_get_option( '' ) . $parent )
				);
		...
		// Value
		case 'txn_value':
			echo esc_html( mdjm_currency_filter( mdjm_format_amount( get_post_meta( $post_id, '_mdjm_txn_total', true ) ) ) );
			break;

		// Status
		case 'txn_status':
			echo esc_html( get_post_meta( $post_id, '_mdjm_txn_status', true ) );
			break;
	} // switch
...
function mdjm_transaction_set_post_title( $post ) {
	...
	<script type="text/javascript">
		jQuery(document).ready(function($) {
			$("#title").val("<?php echo esc_attr( mdjm_get_event_contract_id( $post->ID ) ); ?>");
...
function mdjm_transaction_type_filter_dropdown() {
	...
	<option value="0"><?php echo esc_html__( 'All Transaction Types', 'mobile-dj-manager' ); ?></option>

	<?php
		$current_value = isset( $_GET['mdjm_filter_type'] ) ? sanitize_text_field( wp_unslash( $_GET['mdjm_filter_type'] ) ) : '';

	if ( ! empty( $values ) ) {

		foreach ( $values as $value => $label ) {

			printf( '<option value="%s"%s>%3$s (%3$s)</option>', esc_html( $value ), esc_html( $value == $current_value ? ' selected="selected"' : '' ), esc_html( $label ) );
...
function mdjm_save_txn_post( $post_id, $post, $update ) {
	...
	$trans_type = get_term( sanitize_text_field( wp_unslash( $_POST['mdjm_transaction_type'] ) ), 'transaction-types' ); // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotValidated

	// Set the post data
	$trans_data['ID']          = $post_id;
	$trans_data['post_status'] = $_POST['transaction_direction'] == 'Out' ? 'mdjm-expenditure' : 'mdjm-income'; // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotValidated
	$trans_data['post_date']   = date( 'Y-m-d H:i:s', strtotime( sanitize_text_field( wp_unslash( $_POST['transaction_date'] ) ) ) ); // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotValidated
	$trans_data['edit_date']   = true;

	$trans_data['post_author']   = get_current_user_id();
	$trans_data['post_type']     = 'mdjm-transaction';
	$trans_data['post_category'] = array( sanitize_text_field( wp_unslash( $_POST['mdjm_transaction_type'] ) ) ); // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotValidated

	// Set the post meta
	$trans_meta['_mdjm_txn_status'] = sanitize_text_field( wp_unslash( $_POST['transaction_status'] ) ); // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotValidated
	$trans_meta['_mdjm_txn_source'] = sanitize_text_field( wp_unslash( $_POST['transaction_src'] ) ); // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotValidated
	$trans_meta['_mdjm_txn_total']  = sanitize_text_field( wp_unslash( $_POST['transaction_amount'] ) ); // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotValidated
	$trans_meta['_mdjm_txn_notes']  = sanitize_text_field( wp_unslash( $_POST['transaction_description'] ) ); // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotValidated

	if ( $_POST['transaction_direction'] == 'In' ) { // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotValidated
		$trans_meta['_mdjm_payment_from'] = sanitize_text_field( wp_unslash( $_POST['transaction_payee'] ) ); // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotValidated
	} elseif ( $_POST['transaction_direction'] == 'Out' ) { // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotValidated
		$trans_meta['_mdjm_payment_to'] = sanitize_text_field( wp_unslash( $_POST['transaction_payee'] ) ); // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotValidated
	}
