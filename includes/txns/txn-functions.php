...
function mdjm_get_currency() {
	...
	return apply_filters( 'mdjm_currency', esc_html( $currency ) );
} // mdjm_get_currency
...
function mdjm_get_currency_name( $code = 'GBP' ) {
	...
	return apply_filters( 'mdjm_currency_name', esc_html( $name ) );
...
function mdjm_get_txn( $txn_id ) {
	return mdjm_get_txn_by_id( esc_html( $txn_id ) );
} // mdjm_get_txn
...
function mdjm_get_txn_price( $txn_id ) {
	...
	return esc_html( $mdjm_txn->price );
} // mdjm_get_txn_price
...
function mdjm_get_txn_recipient_id( $txn_id ) {
	...
	return esc_html( $mdjm_txn->recipient_id );
...
function mdjm_update_txn_meta( $txn_id, $data ) {
	...
	foreach ( $data as $key => $value ) {
		...
		if ( 'mdjm_nonce' === $key || 'mdjm_action' === $key ) {
			continue;
		}
	...
	foreach ( $employees as $employee_id => $employee_data ) {
		...
		MDJM()->debug->log_it(
			sprintf(
				...
				mdjm_currency_filter( mdjm_format_amount( $mdjm_txn->price ) )
			),
			true
		);
