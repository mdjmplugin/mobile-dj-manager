...
/** Fix to force use jQuery not Bootstrapper.
  * Copyright 2021 Dan Porter, Jack Mawhinney
*/
		$('<?php echo esc_html( $field ); ?>').on('changeDate', function() {
			var mdjmdate_ = $(this).val();
			var mdjmdate_ = mdjmdate_.split('-').reverse().join('-');
			var mdjmdate_ = mdjmdate_.replace(/-/g,"/");
		$('#<?php echo esc_attr( $args['altfield'] ); ?>').val(mdjmdate_);
		});
	});
	</script>
	<?php
...
/**
 * Class to check for premium addons
 *
 * @since 1.0.4
 */
$CheckMDJMPremium = new MDJM_Check_Premium();
class MDJM_Check_Premium {
