...
function mdjm_setup_content_tags() {
	...
	$content_tags = array(
		...
		array(
			...
			'description' => sprintf( __( 'The remaining %1$s value due for the %2$s', 'mobile-dj-manager' ), mdjm_get_deposit_label(), mdjm_get_label_singular( true ) ),
...
function mdjm_content_tag_contract_date( $event_id = '' ) {
	...
	if ( ! empty( $signed ) ) {
		$return = date( mdjm_get_option( 'short_date_format', 'd/m/Y' ), strtotime( $signed ) );
	} else {
