...
/**
 * Registers and sets up the Mobile DJ Manager custom post types
 *
 * @since 1.3
 * @return void
 */
function mdjm_register_post_types() {
	...
	foreach ( $event_labels as $key => $value ) {
		$event_labels[ $key ] = sprintf( $value, esc_html( mdjm_get_label_singular() ), esc_html( mdjm_get_label_plural() ), mdjm_get_label_plural( true ) );
	}
	...
	if ( mdjm_packages_enabled() ) {
		...
		$package_args = array(
			...
			'description'  => __( 'Equipment Packages for the Mobile DJ Manager plugin', 'mobile-dj-manager' ),
			'public'       => true,
		...
		$addon_labels = apply_filters(
			...
			array(
				...
				'menu_name'          => _x( 'Addons', 'admin menu', 'mobile-dj-manager' ),
		...
		$addon_args = array(
			'labels'       => $addon_labels,
			'description'  => __( 'Equipment Addons for the Mobile DJ Manager plugin', 'mobile-dj-manager' ),
			'public'       => true,
	...
	$playlist_args = array(
		...
		'description'     => __( 'Mobile DJ Manager Playlist Entries', 'mobile-dj-manager' ),
		'show_ui'         => true,
		'show_in_menu'    => false,
...
function mdjm_register_post_statuses() {
	...
	register_post_status(
		'ready to send',
		apply_filters(
			'mdjm_comm_ready_to_send_status',
			array(
	...
	register_post_status(
		...
		apply_filters(
			'mdjm_event_awaitingdeposit_status',
			array(
				'label'                     => sprintf( esc_html__( 'Awaiting %s', 'mobile-dj-manager' ), mdjm_get_deposit_label() ),
				'plural'                    => sprintf( esc_html__( 'Awaiting %s' . 's', 'mobile-dj-manager' ), mdjm_get_deposit_label() ),
				'public'                    => true,
				'exclude_from_search'       => false,
				'show_in_admin_all_list'    => true,
				'show_in_admin_status_list' => true,
	...
	register_post_status(
		'mdjm-income',
		apply_filters(
			'mdjm_transaction_income_status',
			array(
	...
	register_post_status(
		'mdjm-expenditure',
		apply_filters(
			'mdjm_transaction_expenditure_status',
			array(
...
function mdjm_register_taxonomies() {
	...
	$enquiry_source_args = apply_filters(
		'mdjm_enquiry_source_args',
		array(
			'hierarchical'          => false,
			'labels'                => apply_filters( 'mdjm_enquiry_source_labels', $enquiry_source_labels ),
			'description'           => sprintf( esc_html__( 'Track how clients found %s', 'mobile-dj-manager' ), mdjm_get_option( 'company_name', get_bloginfo( 'name' ) ) ),
			'public'                => false,
			'show_ui'               => true,
			'query_var'             => true,
