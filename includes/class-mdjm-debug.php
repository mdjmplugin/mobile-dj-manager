...
class MDJM_Debug	{
	...
	function submit_logs()	{
		...
		/* -- Set log files -- */
		$this->paypal_log = MDJM_PLUGIN_DIR . '/includes/api/api-log/mdjm-pp-ipn-debug.log';
		$this->payfast_log = MDJM_PLUGIN_DIR . '/includes/api/api-log/mdjm-payfast-itn-debug.log';
		$this->php_info_log = MDJM_PLUGIN_DIR . '/includes/admin/phpinfo.html';
		...
		$content .= '<p>' . var_export( $mdjm_options, true ) . '</p>' . "\r\n\r\n";
	...
	function backup_tables( $replace='' )	{
		...
		$data_id = array( MDJM_HOLIDAY_TABLE => 'id' );
		...
		foreach( $tables as $table )	{
			...
			if( $num_rows > 0 )	{
				...
				$mdjm_desc = array( MDJM_HOLIDAY_TABLE => 'Availability Table' );
