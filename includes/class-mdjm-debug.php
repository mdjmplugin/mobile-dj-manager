...
class MDJM_Debug {
	...
	public function __construct() {
		...
		add_action( 'admin_notices', array( &$this, 'log_file_check' ) );
	...
	function log_file_check() {
		...
		if ( isset( $warn ) ) {
			...
			echo '<p>' . esc_html( _n( 'One', 'Some', count( $warn ), 'mobile-dj-manager' ) ) . esc_html__( ' of your log files exceed the specified limit of ' . mdjm_get_option( 'debug_log_size', 2 ) . ' megabytes', 'mobile-dj-manager' ) . '</p>'
			. '<p>' . "\r\n";
			...
			foreach ( $warn as $file => $file_name ) {
				...
				echo '<input type="checkbox" name="delete_files[]" id = "delete_files" value="' . esc_attr( $file_name ) . '" />' .
				'&nbsp;&nbsp;&nbsp;' .
				esc_attr( $file_name ) .
				( $i != count( $warn ) ? '<br />' : '' ) . "\r\n";
