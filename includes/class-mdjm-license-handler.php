...
if ( ! class_exists( 'MDJM_License' ) ) :
	...
	class MDJM_License {
		...
		public function notices() {
			...
			if( ! isset( $_GET['page'] ) || 'mdjm-settings' !== $_GET['page'] ) {
				return;
			}
			...
			$license_error = get_transient( 'mdjm_license_error' );
			...
			if( ! empty( $license_error->error ) ) {
				...
				switch( $license_error->error ) {
					...
					case 'item_name_mismatch' :
						...
						$message = __( 'This license does not belong to the product you have entered it for.', 'mobile-dj-manager' );
						break;
	
					case 'no_activations_left' :
	
						$message = __( 'This license does not have any activations left', 'mobile-dj-manager' );
						break;
	
					case 'expired' :
	
						$message = __( 'This license key is expired. Please renew it.', 'mobile-dj-manager' );
						break;
	
					default :
	
						$message = sprintf( __( 'There was a problem activating your license key, please try again or contact support. Error code: %s', 'mobile-dj-manager' ), $license_error->error );
						break;
