...
function mdjm_process_playlist_upload() {
	...
	foreach ( $entries as $id => $entry ) {
		...
		if ( $response ) {
			...
			$debug[] = sprintf( __( '%1$s by %2$s successfully uploaded.', 'mobile-dj-manager' ), $entry_data['song'], $entry_data['artist'] );
			...
			$debug[] = sprintf( __( '%1$s by %2$s could not be uploaded.', 'mobile-dj-manager' ), $entry_data['song'], $entry_data['artist'] );
		}
	...
	$debug[] = sprintf( __( '%1$d out of %2$d entries successfully uploaded.', 'mobile-dj-manager' ), $i, $count );
