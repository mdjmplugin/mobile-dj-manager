...
function mdjm_email_quote( $event_id, $template_id = '' ) {
	...
	$emails->send( $to_email, $subject, $message, $attachments, sprintf( __( '%1$s quotation and status set to %2$s', 'mobile-dj-manager' ), mdjm_get_label_singular(), mdjm_get_post_status_label( $mdjm_event->post_status ) ) );
...
function mdjm_email_enquiry_accepted( $event_id ) {
	...
	$emails->send( $to_email, $subject, $message, $attachments, sprintf( __( 'Enquiry accepted and %1$s Status set to %2$s', 'mobile-dj-manager' ), mdjm_get_label_singular(), mdjm_get_post_status_label( $mdjm_event->post_status ) ) );

} // mdjm_email_enquiry_accepted

/*
 Email the client that we'd like them to pay the deposit.
 *
 * @since      1.3
 * @param      int             $event_id               The event ID
 * @return     void
...
function mdjm_email_awaitingdeposit( $event_id ) {
	...
	$emails->send( $to_email, $subject, $message, $attachments, sprintf( __( '%1$s Status set to %2$s', 'mobile-dj-manager' ), mdjm_get_label_singular(), mdjm_get_post_status_label( $mdjm_event->post_status ) ) );
...
function mdjm_email_booking_confirmation( $event_id ) {
	...
	$emails->send( $to_email, $subject, $message, $attachments, sprintf( __( 'Contract Signed and %1$s Status set to %2$s', 'mobile-dj-manager' ), mdjm_get_label_singular(), mdjm_get_post_status_label( $mdjm_event->post_status ) ) );

	do_action( 'mdjm_email_booking_confirmation', $mdjm_event );

} // mdjm_email_booking_confirmation

function mdjm_email_manual_invoice( $event_id ) {

	$mdjm_event = mdjm_get_event( $event_id );

	$from_name = mdjm_email_set_from_name( 'email_invoice', $mdjm_event );
	$from_name = apply_filters( 'mdjm_email_from_name', $from_name, 'email_invoice', $mdjm_event );

	$from_email = mdjm_email_set_from_address( 'email_invoice', $mdjm_event );
	$from_email = apply_filters( 'mdjm_email_from_address', $from_email, 'email_invoice', $mdjm_event );

	$client   = get_userdata( $mdjm_event->client );
	$to_email = $client->user_email;

	$subject = mdjm_email_set_subject( mdjm_get_option( 'email_invoice', false ), 'email_invoice' );
	$subject = apply_filters( 'mdjm_email_invoice_subject', wp_strip_all_tags( $subject ) );
	$subject = mdjm_do_content_tags( $subject, $event_id, $mdjm_event->client );

	$attachments = apply_filters( 'mdjm_email_invoice_attachments', array(), $mdjm_event );

	$message = mdjm_get_email_template_content( mdjm_get_option( 'email_invoice', false ), 'email_invoice' );
	$message = mdjm_do_content_tags( $message, $event_id, $mdjm_event->client );
	...
	$headers = apply_filters( 'mdjm_email_invoice_headers', $emails->get_headers(), $event_id, $mdjm_event->client );
	$emails->__set( 'headers', $headers );

	$emails->__set( 'track', apply_filters( 'mdjm_track_email_invoice', mdjm_get_option( 'track_client_emails' ) ) );

	$emails->__set( 'copy_to', mdjm_email_maybe_send_a_copy( $to_email, $event_id ) );

	do_action( 'mdjm_email_invoice', $mdjm_event );
