...
function mdjm_create_merchant_fee_txn( $gateway_data ) {
	...
	$txn_data = apply_filters(
		...
		array(
			...
			'post_type'   => 'mdjm-transaction',
			'post_title'  => sprintf(
				__( '%1$s Merchant Fee for Transaction %2$s', 'mobile-dj-manager' ),
				$gateway,
				$gateway_data['txn_id']
			),
...
function mdjm_admin_payment_notice( $txn_data ) {
	...
	$content .= '<p>' . __( 'A payment has just been received via Mobile DJ Manager', 'mobile-dj-manager' ) . '</p>' . "\n";
	$content .= '<hr />' . "\n";
	...
	mdjm_add_journal(
		array(
			...
			'comment_content' => sprintf(
				__( '%1$s of %2$s received via %3$s', 'mobile-dj-manager' ),
				$type,
