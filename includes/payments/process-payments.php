...
function mdjm_process_payment_form() {
	...
	do_action( 'mdjm_payment_before_gateway', sanitize_text_field( $_POST ), $payment_data );
...
function mdjm_payment_form_validate_fields() {
	...
	$event_id = isset( $_POST['event_id'] ) ? absint( wp_unslash( sanitize_text_field( $_POST['event_id'] ) ) ) : 0;
...
function mdjm_get_payment_form_client( $client_id = 0 ) {
	...
	if ( $client ) {
		...
		$client_data['id']           = isset( $client->ID ) ? sanitize_text_field( $client->ID ) : '';
