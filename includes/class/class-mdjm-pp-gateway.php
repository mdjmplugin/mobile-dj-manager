	...
	class MDJM_PayPal	{
		...
		*/
		function pp_form( $post )	{
			...
			$pp_email = $mdjm_settings['paypal']['paypal_email'];
			if( isset( $mdjm_settings['paypal']['enable_sandbox'] ) )	{
				$pp_api = 'www.sandbox.paypal.com/cgi-bin/webscr';
				$pp_email = !empty( $mdjm_settings['paypal']['sandbox_email'] ) ? $mdjm_settings['paypal']['sandbox_email'] : $mdjm_settings['paypal']['paypal_email'];
			}
			...
			$pp_form .= '<input type="hidden" name="return" value="' . $mdjm->get_link( $mdjm_settings['paypal']['redirect_success'] ) . 'pp_action=completed&event_id=' . $post->ID . '">' . "\n";
			$pp_form .= '<input type="hidden" name="cancel_return" value="' . $mdjm->get_link( $mdjm_settings['paypal']['redirect_cancel'] ) . 'pp_action=cancelled&event_id=' . $post->ID . '">' . "\n";
			$pp_form .= '<input type="hidden" name="currency_code" value="' . $mdjm_settings['payments']['currency'] . '">' . "\n";
			$pp_form .= '<input type="hidden" name="bn" value="PP-BuyNowBF:' . $mdjm_settings['paypal']['paypal_button'] . ':NonHosted">' . "\n";
			...
			if( isset( $mdjm_settings['payments']['enable_tax'], $mdjm_settings['payments']['tax_type'], $mdjm_settings['payments']['tax_rate'] ) )	{
				...
				elseif( $mdjm_settings['payments']['tax_type'] == 'fixed' )	{
					$tax = 'tax';
				}
				$pp_form .= '<input type="hidden" name="' . $tax . '" value="' . $mdjm_settings['payments']['tax_rate'] . '">' . "\n";
			}

			if( !empty( $mdjm_settings['paypal']['checkout_style'] ) )	{
				$pp_form .= '<input type="hidden" name="page_style" value="' . $mdjm_settings['paypal']['checkout_style'] . '">' . "\n";
			}
			
			$pp_form .= '<input type="hidden" name="on0" value="Paying for">' . "\n";
			
			if( $mdjm_settings['payments']['form_layout'] == 'horizontal' )	{
				$pp_form .= '<table>' . "\n";
				$pp_form .= '<tr valign="middle">' . "\n";
				$pp_form .= '<td>';
			}
			
			$pp_form .= $mdjm_settings['payments']['payment_label'] . "\n";
			if( $mdjm_settings['payments']['form_layout'] == 'vertical' )	{
				$pp_form .=  '<br />' . "\n";
			}
			else	{
				$pp_form .= '</td>' . "\n";
			...
			$pp_form .= '>' . MDJM_DEPOSIT_LABEL . ' ' . display_price( get_post_meta( $post->ID, '_mdjm_event_deposit', true ) ) . '</option>' . "\n";
			...
			$pp_form .= '>' . MDJM_BALANCE_LABEL . ' ' . display_price( $balance ) . '</option>' . "\n";
			$pp_form .= '</select>' . "\n";
			...
			$pp_form .= '<input type="image" src="https://www.paypalobjects.com/en_GB/i/btn/' . $mdjm_settings['paypal']['paypal_button'] . 
